    ...
    <main className="w-full bg-white pb-24">
      <header className="py-24 border-b border-outline bg-slate-50">
        <div className="max-w-screen-xl mx-auto px-6 text-center">
          <h2 className="text-5xl md:text-7xl font-serif text-primary mb-8 font-light tracking-tight">{t('journalArchive')}</h2>
          <p className="text-xl font-serif text-on-surface-variant max-w-2xl mx-auto leading-relaxed opacity-80">
      ...
      <div className="max-w-screen-xl mx-auto px-6 -mt-8">
        <div className="bg-white border border-outline shadow-xl flex items-center px-8 py-6 mb-24">
          ...
          <input 
            className="bg-transparent border-none focus:ring-0 w-full font-serif text-xl" 
          ...
          <aside className="md:col-span-4 space-y-12">
            <div className="border-t-2 border-primary pt-8">
              ...
              <ul className="space-y-6">
                <li><a href="#" className="font-serif text-lg hover:text-accent transition-colors">Centennial Flight Review (2026)</a></li>
                <li><a href="#" className="font-serif text-lg hover:text-accent transition-colors">Post-9/11 Security Jurisprudence</a></li>
                <li><a href="#" className="font-serif text-lg hover:text-accent transition-colors">The Chicago Convention Retrospective</a></li>
              </ul>
            </div>
          </aside>
        </div>
  ...
  return (
    <section>
      <div className="border-b border-outline pb-4 mb-10 flex justify-between items-baseline">
        ...
        <span className="font-serif text-slate-400">{year}</span>
      </div>
      <div className="grid grid-cols-1 gap-4">
        {issues.map((issue: string) => (
          <Link key={issue} to={vol === "3" ? "/article?id=sahil" : "/article"} className="group flex items-center justify-between p-6 hover:bg-slate-50 border border-transparent hover:border-outline transition-all">
            <span className="text-xl font-serif group-hover:text-accent transition-all">{issue}</span>
